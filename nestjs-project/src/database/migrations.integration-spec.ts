...
import { CreateVideos1789028560279 } from './migrations/1789028560279-CreateVideos';
...
// DROP TABLE ... CASCADE removes the column but never the enum TYPE itself —
// it must be dropped explicitly or a re-run's CREATE TYPE fails with
// "already exists" against a DB that already saw this suite run once.
...
describe('Database migrations (integration)', () => {
  ...
  it('should revert the videos migration and remove only the videos table', async () => {
    ...
    const remaining = await dataSource.query<{ table_name: string }[]>(
      `SELECT table_name FROM information_schema.tables
       WHERE table_schema = 'public'
         AND table_name = ANY($1::text[])`,
      [['channels', 'users', 'refresh_tokens', 'verification_tokens']],
    );
